...
**Advisor library** (`advisors/`): Named advisor profiles generated by `incarnate`. Each profile contains a contributor's background and thinking style (cognitive patterns, attention patterns, reasoning strengths, conversation dynamics). The ideas skill role-plays as a selected advisor during brainstorming.
