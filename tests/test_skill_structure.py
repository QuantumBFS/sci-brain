...
def test_download_ref_targets_dot_knowledge():
    text = _read("how-to-download-ref")
...
def test_download_ref_uses_vendored_helpers_path():
    text = _read("how-to-download-ref")
    # Operational helpers should be invoked from skills/how-to-download-ref/helpers/.
...
def test_survey_delegates_kb_building():
    text = _read("survey")
    assert "how-to-build-kb" in text
    assert "append_bibtex.py" not in text  # moved to how-to-build-kb


# ---- how-to-build-kb ----

def test_build_kb_uses_dot_raw_json_path():
    text = _read("how-to-build-kb")
    assert ".raw/" in text
    assert "resolve_kb" in text


def test_build_kb_invokes_append_bibtex_for_dedup():
    text = _read("how-to-build-kb")
    assert "append_bibtex.py" in text


def test_build_kb_regenerates_index_and_writes_notes():
    text = _read("how-to-build-kb")
    assert "index.py" in text
    assert "INDEX.md" in text
    assert "NOTES.md" in text
    assert "Step 4" not in text  # no survey-step numbering leaks
...
def test_brainstorm_ideas_delegates_ideas_report():
    text = _read("brainstorm-ideas")
    assert "how-to-write-ideas-report" in text
    assert "Ideas Report Mode" not in text
...
def test_incarnate_invokes_know_me_better_or_download_ref():
    text = _read("create-advisor")
    # The advisor KB is populated by /know-me-better or /how-to-download-ref
...
def test_paper_reviewer_has_frontmatter():
    ...
    assert "name: review-paper" in text
...
def test_paper_reviewer_repairs_refs_via_download_ref():
    # Never invent BibTeX from memory; repair via how-to-download-ref.
    text = _read("review-paper")
    assert "how-to-download-ref" in text
...
def test_paper_reviewer_does_not_collide_with_writer_description():
    ...
    # from write-paper (drafting) and survey report mode (field assessment).
    text = _read("review-paper")
