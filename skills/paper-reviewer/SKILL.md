...
- **Confirm non-ok records.** Use the helper's severity-ranked output as the starting point for the reference / fact-check table. Manually confirm every `mismatch` or `unverifiable` entry through **CrossRef → Semantic Scholar → MCP → WebFetch** before reporting it; Semantic Scholar is a screening source, not the final authority. Flag broken, missing, or confirmed mismatches and offer repair via `/download-ref` (it owns `references.bib` appends and metadata fetching).
