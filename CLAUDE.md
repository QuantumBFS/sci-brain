...
- **download-ref** — Adds one or a few new arXiv IDs / DOIs to an existing registry: fetches metadata, downloads + renders the PDF, proposes a cite key (with confirmation), appends to `references.bib`, and inserts a row under `summary.md`. Triggered manually or via end-of-session prompts in `/ideas`, `/survey`, `/researchstyle`.
