...
Sixteen skills in `skills/`, each defined by a `SKILL.md` with YAML frontmatter + instructions:
...
- **qe-epc** — Prepares, runs, monitors, debugs, and interprets Quantum ESPRESSO native DFPT electron-phonon superconductivity workflows (`pw.x`, `ph.x`, `q2r.x`, `matdyn.x`, `alpha2f.x`, `lambda_tetra`, `alpha2F(omega)`, `lambda`, `omega_log`, `Tc`). Keeps the main runbook in `SKILL.md` and concrete PdH-style source files under `skills/qe-epc/source/`; not an EPW workflow.
- **qmcpack-dmc** — Prepares, runs, monitors, debugs, and interprets Quantum ESPRESSO → QMCPACK Slater-Jastrow VMC/DMC workflows (`pw.x`, `pw2qmcpack.x`, ESHDF/einspline orbitals, Jastrow optimization, twist averaging, fixed-node DMC, timestep checks, scalar summaries, and twist-parallel launch tuning). Keeps the main runbook in `SKILL.md`, the run-preparation checklist in `source/prepare-qmcpack-todo.md`, and source templates under `skills/qmcpack-dmc/source/`.
