...
- **paperwrite** — Use when drafting or revising an actual scientific manuscript (journal article, PRL/Nature-style letter, arXiv preprint). Encodes the von Delft / Martinis workflow: figures first → telegram outline → body → polish abstract+intro+conclusions last. Includes a figure rulebook (beamer-safe, greyscale-safe, dimensionless, never arbitrary units), a notation rulebook, sentence-level rules, and a pre-submission checklist. Distinct from `writer` — paperwrite is for real manuscripts with results, not for ideas reports.
- **researchstyle** — Indexes a paper collection (your own or another researcher's) into registry format. Output lands at `<registry-root>/<slug>/` (default `personal`). Auto-called from `/ideas` during background setup, or run standalone.
- **fetch-papers** — Run after a registry exists. Reads `references.bib`, downloads arXiv PDFs (with arXiv-preprint fallback for paywalled DOIs), renders each PDF to markdown via pymupdf4llm, and writes the result alongside the registry under `.raw/`, `.figures/`, and per-paper `<id>_<slug>.md` files.
- **download-ref** — Single-shot counterpart to `fetch-papers`. Adds one or a few new arXiv IDs / DOIs to an existing registry: fetches metadata, downloads + renders the PDF, proposes a cite key (with confirmation), appends to `references.bib`, and inserts a row under `summary.md`. Reuses `fetch-papers/helpers/`. Triggered manually or via end-of-session prompts in `/ideas`, `/survey`, `/researchstyle`.
