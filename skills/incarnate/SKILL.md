---
name: incarnate
description: Use when onboarding or updating a contributor as an advisor, importing Markdown conversation exports into an advisor profile, or extracting thinking patterns and logic jumps from classified conversations — analyzes conversation history and synthesizes a named advisor profile for the brainstorm-ideas advisor library
---
...
Then follow the adapted conversation-dump Phases 2–4 on these files: classify them into `docs/dialog/md-import/<topic>/`, deeply tag all six dimensions, and persist the enriched JSON reports in those topic folders before pattern extraction begins.
...
This workflow consumes the tagged JSON reports produced by `conversation-dump` (including parsed Markdown imports) and writes two intermediate artifacts: recurring trigger→reaction patterns and user-confirmed logic jumps.
