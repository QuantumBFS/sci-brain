...
- **autoresearch** — Dispatcher for the autoresearch pipeline (`autoresearch-topics` → `autoresearch-metrics` → `autoresearch-db` → `autoresearch-validator` → `autoresearch-run`). Reads `research/STATE.md`, verifies stage gate artifacts, routes to the right stage skill. Start or resume any autoresearch project here.
- **autoresearch-topics** — Stage 1: brainstorms topics in a domain scored on Checkable/Cheap/Headroom/Publishable; user picks; writes `topics.md`.
- **autoresearch-metrics** — Stage 2: attaches primary/guard score metrics (with computation cost and gaming risks) to each topic in `topics.md`.
- **autoresearch-db** — Stage 3: insight-coverage-driven reference downloads (via `download-ref`), distillation into user-selected `research/INSIGHTS.md`, domain database, pinned reference implementations, `research/CATALOG.md`. Owns the survey gate.
- **autoresearch-validator** — Stage 4: publishable bar in `GOAL.md`, sealed gitignored holdout, Docker-canonical `validate` CLI with rich JSON errors, negative-control strictness self-test. Owns the validator gate.
- **autoresearch-run** — Stage 5: the loop — batches of attempts (worktree + `LOG.md` each, validator-scored under a hard time limit), reflection reports in `docs/discussion/`, soft-gated by `authorized_rounds`.
