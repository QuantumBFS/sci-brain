...
The eight-guideline review rubric backing `paper-reviewer/SKILL.md`, expanded into checkable items. Use it as the per-pass checklist when reviewing a manuscript. Guidelines 1–3, 5, and 7 restate `paper-writer` authoring rules as *review* checks — consult `skills/paper-writer/references.md` for the reasoning. Guidelines 4, 6, and 8 are specific to reviewing.
...
- [ ] Every `mismatch` or `unverifiable` record was manually confirmed through CrossRef → Semantic Scholar → MCP → WebFetch before reporting it.
